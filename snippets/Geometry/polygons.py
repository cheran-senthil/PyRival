...
area = lambda *p: abs(
    sum(i[0] * j[1] - j[0] * i[1] for i, j in zip(p, p[1:] + p[:1]))) / 2

insideCircle = lambda p, c, r: sum(i * i - j * j for i, j in zip(p, c)) < r * r

rInCircle = lambda a, b, c: area(a, b, c) / (perimeter(a, b, c) / 2)

rCircumCircle = lambda a, b, c: (dist(a, b) * dist(b, c) * dist(c, a)) / (
    4 * area(a, b, c))
