...
minor = lambda mat, i, j: [
    row[:j] + row[j + 1:] for row in (mat[:i] + mat[i + 1:])
]

mat_add = lambda *mat: [[sum(elements) for elements in zip(*row)]
                        for row in zip(*mat)]

mat_sub = lambda mat1, mat2: [[i - j for i, j in zip(*row)] for row in zip(
    mat1, mat2)]

mat_mul = lambda mat1, mat2: list(
    map(
        lambda row: list(
            map(lambda *column: sum(map(op.mul, row, column)), *mat2)), mat1))
...
def inverse(m):
    ...
    return transpose([[(pow(-1, i + j) * det(minor(m, i, j))) / determinant
                       for j in range(len(m))] for i in range(len(m))])


linear_recurrence = lambda trans_matrix, known_values, k: mat_mul(
    mat_pow(trans_matrix, k), known_values)
